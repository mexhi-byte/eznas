...
export async function handleFileRoutes(ctx: FileRouteContext): Promise<boolean> {
    ...
    json(res, 200, {
      ...
      entries: entries
        .map((e) => ({
           ...
           */
          kind: e.name === ".recycle" ? "bin" : e.type === "DIRECTORY" ? "dir" : files.kindOf(String(e.name)),
        }))
        .sort((a, b) =>
          a.type === b.type ? String(a.name).localeCompare(String(b.name)) : a.type === "DIRECTORY" ? -1 : 1,
        ),
    });
    return true;
  }
  ...
  if (path === "/api/files/content" || path === "/api/files/download") {
    ...
    if (!wantsDownload && size > files.MAX_INLINE) {
      throw new Error(
        `This file is ${(size / 1024 ** 2).toFixed(0)} MB, too large to open in the browser. Download it instead.`,
      );
    }
